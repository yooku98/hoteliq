...
export default function MaintenanceList({ tickets, onUpdateStatus }) {
  ...
  return (
    <Panel title="Open maintenance tickets" badge={<Badge tone="coral">{open.length} open</Badge>}>
      {open.length === 0 ? (
            ...
            >
              <div className="min-w-0">
                ...
                <div className="text-[11px] text-ink3">
                  Reported by {t.reported_by_staff?.name ?? 'staff'} · {ageLabel(t.created_at)}
                </div>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                <span
                  className={`text-[10px] font-semibold px-2 py-0.5 rounded-md ${
                    t.status === 'in_progress' ? 'bg-gold-light text-gold' : 'bg-coral-light text-coral'
                  }`}
                >
                  {t.status.replace('_', ' ')}
                </span>
                {t.status === 'open' ? (
                  ...
                  <button
                    ...
                    className="text-[11px] text-green font-medium"
                  >
                    Resolve
                  </button>
                )}
